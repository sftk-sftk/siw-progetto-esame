...
public class AuthConfiguration { // Spring Authentication configuration class
	 ...
	 * oAuth2AuthenticationSuccessHandler() { return new
	 * OAuth2AuthenticationSuccessHandler(); // Register your custom success handler
	 * }
	 */
		...
		httpSecurity.csrf(csrf -> csrf.disable()) // Disabilita CSRF se necessario, o lo configura come richiesto
				...
				.authorizeHttpRequests(authorize -> authorize
						...
						.requestMatchers(HttpMethod.GET, "/", "/oauth2/**", "/index", "/register", "/registerAdmin",
								...
								"/images/**", "/images/prodotti/**", "favicon.ico", "/error", "/catalogo", "/catalogo10Euro", "/catalogoDelMomento",
								"/prodotto/**")
						.permitAll()
						/*
						 ...
						 */
						.requestMatchers(HttpMethod.POST, "/register", "/registerAdmin", "/login").permitAll()
						/*
						 * solo gli utenti autenticati con ruolo HOST e INTERMEDIATE_HOST possono
						 * accedere a risorse con path
						 */
				...
				/*
				 * OAUTH: here it's defined the OAuth2.0 login .oauth2Login(oauth2Login ->
				 * oauth2Login.loginPage("/login")
				 * .successHandler(oAuth2AuthenticationSuccessHandler())) // this is a
				 * constructor of a specific // class (the other in this same
				 */ // package)
